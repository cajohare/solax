...
def B_model_constant(B0=1e5): # Gauss
...
def B_model_seismic(B_rad=5e3,B_tach=50.0,B_outer=3.0,r0_12=0.712,r0_31=0.96,d2=0.02,d3=0.035):
    # from https://arxiv.org/pdf/astro-ph/0203107.pdf
...
def B_model_seismic_rad(B_rad=5e3,B_tach=50.0,B_outer=3.0,r0_12=0.712,r0_31=0.96,d2=0.02,d3=0.035):
    # from https://arxiv.org/pdf/astro-ph/0203107.pdf
    B12 = zeros(shape=size(r))
    lamb = 10*r0_12+1
    region1 = r<r0_12
...
def B_model_seismic_tach(B_rad=5e3,B_tach=50.0,B_outer=3.0,r0_12=0.712,r0_31=0.96,d2=0.02,d3=0.035):
    # from https://arxiv.org/pdf/astro-ph/0203107.pdf
    B12 = zeros(shape=size(r))
    lamb = 10*r0_12+1
    region1 = r<r0_12
...
def B_model_seismic_outer(B_rad=5e3,B_tach=50.0,B_outer=3.0,r0_12=0.712,r0_31=0.96,d2=0.02,d3=0.035):
    # from https://arxiv.org/pdf/astro-ph/0203107.pdf
    B12 = zeros(shape=size(r))
    lamb = 10*r0_12+1
    region1 = r<r0_12
...
def AxionFlux_Lplasmon(gag,omega_vals,B_model):
    ...
    for i in range(0,nvals):
        ...
        i_r = argmin(abs(w-wp))
        r0 = r_keV[i_r]
        dPhi_pl[i] = keV_2_s*1/(12*pi*AU_cm**2.0)*(r0**2.0)*(w*w)*gag_keV**2.0*(B_model[i_r]**2.0)/(exp(w/T[i_r])-1)*(1/abs(dwp[i_r-1]))
    return dPhi_pl
...
def smear(E,dR,sig_E):
    ...
    dR_smeared[0:imax] = dR_smeared[0:imax]*\
                        trapz(dR[0:imax],E[0:imax])/trapz(dR_smeared[0:imax],E[0:imax])
    return dR_smeared
#------------------------------------------------------------------------------#
...
dN_test = PhotonNumber_gag_BufferGas(Ei,AxionFlux_Primakoff_PlasmonCorrection(1e-10,Ei),1e-6,0.1)


def BinnedPhotonNumberTable_Vacuum(m_vals,Ei,E_bins,Flux10,\
                           Bfield=2.5,Exposure=3.0,Length=20.0,\
                           N_bores=8,BoreDiameter=60.0,eps_D=0.7,eps_T=0.8,res_on=True):
    ...
    # Eres = True/False, whether to do energy resolution integral or not
...
def BinnedPhotonNumberTable_AllModes(m_vals,Ei,E_bins,Flux10,\
    ...
    # Vacuum part
    Signal_10_vacuum,R0 = BinnedPhotonNumberTable_Vacuum(m_vals,Ei,E_bins,Flux10,Exposure=Exposure_1,\
                            res_on=False,Bfield=Bfield,Length=Length,\
                         N_bores=N_bores,BoreDiameter=BoreDiameter,eps_D=eps_D,eps_T=eps_T)
    ...
    Signal_10_BG1 = BinnedPhotonNumberTable_BufferGas(m_vals,Ei,E_bins,Flux10,pressure_vals,Exposure=Exposure_2,res_on=res_on,\
    ...
    Signal_10_BG2 = BinnedPhotonNumberTable_BufferGas(m_vals,Ei,E_bins,Flux10,pressure_vals,Exposure=Exposure_3,res_on=res_on,\
                                                    Bfield=Bfield,Length=Length,T_operating=T_operating,\
                                                 N_bores=N_bores,BoreDiameter=BoreDiameter,eps_D=eps_D,eps_T=eps_T)
